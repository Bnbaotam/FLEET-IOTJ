...
class LoggingFedRep(FedRepCustom):
    ...
    def aggregate_evaluate(self, server_round, results, failures):
        ...
        if not results:
            print(f"[Server - FedRepTraj] Round {server_round} - No evaluation results received.")
            return None, {}

        scores = []
        print(f"\n[Server - FedRepTraj] Round {server_round} - Client Evaluation Results:")
...
def main():
    parser = argparse.ArgumentParser(description="Federated Server with FedRepTraj Strategy")
    parser.add_argument("--num_rounds", type=int, default=5, help="Number of federated training rounds")
