...
class TrajectoryClient(fl.client.NumPyClient):
    def __init__(self, cfg, data_fraction=1.0):
        ...
        self.rep_keys = sorted(list([key for key in self.model.state_dict().keys() if key.startswith("encoder")]))
    ...
    def get_parameters(self, config):
        """
        Sends representation layers of model to the central server
        """
        # I am only returning a list with the parameters of the representation
        # This will make it so I only receive parameters of length equal to the rep_keys
        state_dict = self.model.state_dict()
        return [state_dict[key].cpu().numpy() for key in self.rep_keys]
        # return [val.cpu().numpy() for val in self.model.state_dict().values()]

    def set_parameters(self, parameters):
        """
        Receives representation weights from central server and update local model, ONLY the representation layers.
        """

        assert len(parameters) == len(self.rep_keys)

        state_dict = self.model.state_dict()
        for key, param in zip(self.rep_keys, parameters):
            if key in self.rep_keys:
                state_dict[key] = torch.from_numpy(np.asarray(param))
    ...
    def fit(self, parameters, config):
        ...
        for name, param in self.model.named_parameters():
            if name in self.rep_keys:
                param.requires_grad = False
            else:
                param.requires_grad = True
        
        # Perform one local head-only training epoch (number of epochs is 5)
        #Reduced to 1 due to time and fairness to other strategies
        ...
        #Perform one local representation-only training epoch (number of epochs is 1)
        self.model.optimizer.zero_grad()
        loss = fl_local_train(
            self.cfg, model=self.model, train_loader=self.train_loader, num_epochs=1
        )
        ...
        # Return *representation* updated parameters and metrics to server
...
def main():
    parser = argparse.ArgumentParser(description="Federated Learning Client for FedRepTraj")
    ...
    parser.add_argument("--server_address", type=str, default="127.0.0.1:9090", help="Flower server address")
