...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config_file", type=str, default='config/TP/FlowChain/cuip.yml',
...
def train(cfg: CfgNode, save_model=True) -> None:
    ...
    # ——— DEBUG: peek at first batch ———
    first_batch = next(iter(data_loader))
    print("Data Dict Keys:", list(first_batch.keys()))
    
    for k, v in first_batch.items():
        if isinstance(v, torch.Tensor):
            print(f"  {k}: shape={v.shape}, dtype={v.dtype}, min={v.min().item():.3f}, max={v.max().item():.3f}")
        # else:
        #     print(f"  {k}: {type(v)} = {v}")
