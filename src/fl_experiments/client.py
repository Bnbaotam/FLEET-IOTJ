...
class TrajectoryClient(fl.client.NumPyClient):
    def __init__(self, cfg, data_fraction=1.0):
        ...
        # ── DEBUG ──────────────────────────────────────────────
        print(f"[DEBUG] DATASET_NAME : {cfg.DATA.DATASET_NAME}")
        print(f"[DEBUG] DATA root    : {cfg.DATA.PATH}")          # or cfg.DATA.ROOT / cfg.DATA.DIR
        print(f"[DEBUG] OUTPUT_DIR   : {cfg.OUTPUT_DIR}")
        print(f"[DEBUG] Full cfg.DATA: {cfg.DATA}")
        # ───────────────────────────────────────────────────────
...
def main():
    ...
    parser.add_argument("--server_address", type=str, default="00.00.00.00:0000", help="Flower server address (IP:PORT)")
    ...
    # print(f"[INFO] AMP Enabled: {cfg.SOLVER.USE_AMP}")
