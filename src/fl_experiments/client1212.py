...
class TrajectoryClient(fl.client.NumPyClient):
    ...
    def get_parameters(self, config):
        # return [val.cpu().numpy() for val in self.model.model.state_dict().values()]  # changed this code
        ######################################################
        try:                                          
    ...
    def set_parameters(self, parameters):
        # state_dict = self.model.model.state_dict() #changed this line
        #########################################################
        try:
            state_dict = self.model.model.state_dict()
        except AttributeError:
        ...
        # self.model.model.load_state_dict(new_state_dict, strict=True)  # changed this line
        #################################################################
        try:
    ...
    def fit(self, parameters, config):
        ...
        if current_ade < self.best_ade:
            ...
            #############################################
            torch.save({
                # "model_state_dict": self.model.model.state_dict(),  # changed this line
...
def main():
    ...
    parser.add_argument("--server_address", type=str, default="127.0.0.1:1030", help="Flower server address (host:port)")
